...
from calculating.CalculatingTogetherTimeScore import CalculatingTogetherTimeScore


def execution(configFile):
    #DEFINE THE FILE THAT WILL KEEP THE RESULT DATA
    resultFile = open(FormatingDataSets.get_abs_file_path(configFile + 'core03_APENASTIMESCORE.txt'), 'w')
    ...
    calc = CalculatingTogetherTimeScore(myparams, nodeSelection.nodesNotLinked)
    
    ordering = calc.ordering(len(nodeSelection.eNeW))
    
    calc.AnalyseNodesInFuture(ordering, myparams.testGraph)
    
    resultFile.write(calc.get_TotalSucess())
