...
def get_partOfWeightCalculating(graph, database, pair, cn):
    ...
    cnWJC = (weightsA["JC"] + weightsB["JC"]) / 2
    ...
    aajc = 1 / (numpy.log10(Total_zjc)   + 0.00001)
    ...
    return {'cnWts02': cnWts02, 'cnWts05': cnWts05, 'cnWts08': cnWts08, 'aaWts02': aaWts02, 'aaWts05':aaWts05, 'aaWts08':aaWts08, 'cnWJC': cnWJC, 'aaWJC': aaWJC }
...
def calculatingWeights(graph, nodesnotLinked, database, calculatingFile):
    ...
    pdb.create('node1', 'node2', 'cnWTS02','cnWTS05','cnWTS08', 'aaWTS02', 'aaWTS05', 'aaWTS08', 'cnWJC', 'aaWJC')
    ...
    for pair in nodesnotLinked:
        ...
        pdb.insert(str(pair[0]), str(pair[1]), CNWts02Feature, CNWts05Feature, CNWts08Feature, AAWts02Feature, AAWts05Feature, AAWts08Feature,CNWJCFeature, AAWJCFeature  )   
    pdb.commit()
    return pdb;
...
def get_ordering(calcDb, topRank):
    ...
    cnWJC = sorted( list({ 'node1': r['node1'], 'node2': r['node2'], 'value' : r['cnWJC']} for r in calcDb)  , key=lambda value: value['value'], reverse=True)
    aaWJC = sorted( list({ 'node1': r['node1'], 'node2': r['node2'], 'value' : r['aaWJC']} for r in calcDb)  , key=lambda value: value['value'], reverse=True)
    
    
    for item in range(topRank):
        orderedResults.append({'cnWTS02' : cnWTS02[item], 'cnWTS05':cnWTS05[item], 'cnWTS08': cnWTS08[item], 'aaWTS02':aaWTS02[item], 'aaWTS05' : aaWTS05[item],'aaWTS08' : aaWTS08[item], 'cnWJC' : cnWJC[item], 'aaWJC' : aaWJC[item] })
...
def get_analyseNodesInFuture(ordering, TestGraph):
    ...
    for nodeToCheck in ordering:
        ...
        if (TestGraph.has_edge(nodeToCheck['cnWJC']['node1'],nodeToCheck['cnWJC']['node2'])):
            cnWJC.append([  nodeToCheck['cnWJC']['node1'],nodeToCheck['cnWJC']['node2'], 1 ])
        else:
    ...
    return {'cnWTS02' : len([result for n1,n2,result in cnWTS02 if result == 1]), 
            'cnWTS05' : len([result for n1,n2,result in cnWTS05 if result == 1]), 
            'cnWTS08' : len([result for n1,n2,result in cnWTS08 if result == 1]), 
            'aaWTS02' : len([result for n1,n2,result in aaWTS02 if result == 1]), 
            'aaWTS05' : len([result for n1,n2,result in aaWTS05 if result == 1]), 
            'aaWTS08' : len([result for n1,n2,result in aaWTS08 if result == 1]),
            'cnWJC' : len([result for n1,n2,result in cnWJC if result == 1]),
            'aaWJC' : len([result for n1,n2,result in aaWJC if result == 1])
             }
