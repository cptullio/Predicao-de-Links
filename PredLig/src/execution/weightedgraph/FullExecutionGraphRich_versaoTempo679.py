...
def get_partOfWeightCalculating(graph, database, pair, cn):
    ...
    cnWts02 = (weightsA["CTS02"] + weightsB["CTS02"]) / 2
    cnWts05 = (weightsA["CTS05"] + weightsB["CTS05"]) / 2
    cnWts08 = (weightsA["CTS08"] + weightsB["CTS08"]) / 2
    ...
    for z in all_neighbors(graph,cn):
        ...
        if (len(row) > 0):
            weightsZ = row[0]
            Total_zts02 = Total_zts02 + weightsZ["CTS02"]
            Total_zts05 = Total_zts05 + weightsZ["CTS05"]
            Total_zts08 = Total_zts08 + weightsZ["CTS08"]
    ...
    for node in sortedNodes:
        ...
        for other in others:
            if graph.has_edge(node, other):
                ...
                decayfunction02 = (1 - 0.6) ** jc
                decayfunction05 = (1 - 0.7) ** jc
                decayfunction08 = (1 - 0.9) ** jc
                
                
                CTS02 = total_publications * decayfunction02
                CTS05 = total_publications * decayfunction05
                CTS08 = total_publications * decayfunction08
...
def execution(configFile):
    ...
    resultFile = open(FormatingDataSets.get_abs_file_path(configFile + 'wTScore03_060709.txt'), 'w')
