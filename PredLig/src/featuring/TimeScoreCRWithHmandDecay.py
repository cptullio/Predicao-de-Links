...
class TimeScoreCRWithHmandDecay(FeatureBase):
    '''
    TimeScore modified by Carlos and Ronaldo
    '''
    def __repr__(self):
        return 'tsCRWithDecay'
    

    def __init__(self):
        super(TimeScoreCRWithHmandDecay, self).__init__()
    ...
    def execute(self, node1, node2):
        ...
        for pair_common_neighbor in pair_common_neighbors:
            ...
            decaykeywordjc = (1 - float(0.5)) ** jcKeyworkds
            control = (abs( max(list(timesofLinks[0])) - max(list(timesofLinks[1])))  + 1) * decaykeywordjc  
            
            
            tswithKeywords =  ( (hm * decayfunction) / control   )
